...
export const useSku = (
  ...
  const defaultValue =
    useModuleContext((s) => s.moduleStore.defaultValue) || {};
  const couponQuery = useModuleContext((s) => s.moduleStore[couponKey]) || noop;
  const promotionQuery =
    useModuleContext((s) => s.moduleStore[promotionKey]) || noop;
  const setModuleStore = useModuleContext((s) => s.setModuleStore);
    ...
    const arr = specList.map((item) => {
      const { specValueValue } =
        item.skuOption?.find((c) =>
          selectObj.skuName?.includes(c.specValueValue),
        ) || {};
  ...
  const isTrue = (str: string, list: Array<string>) => {
    return list.every((item) => {
      return (
        str.includes(`/${item}/`) || str.endsWith(item) || str.startsWith(item)
      );
    });
  };

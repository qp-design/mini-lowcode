...
export function useOrderPay(selfPickupKey: string) {
  ...
  const _orderAddressInfo = useModuleContext(
    (s) => s.moduleStore._orderAddressInfo,
  ); //地址信息
  ...
  const paramsDataHandle = (paramsInfo: object) => {
    ...
    const {
      ...
      goodsReceiptPhone,
      areaCode,
    } = orderAddressInfoMix(paramsInfo);
    return _orderDomainStr.map(
      (item: typeof initialValueOrder, index: number) => {
        return {
          ...
          packageList: [
            {
              ...
              shoppingGoodsIdList: (_contractGoodsList[index] || []).map(
                (item: any) => item.shoppingGoodsId,
              ),
  ...
  const onSubmit = async (cb: () => void, value: any) => {
    ...
    // 优惠信息
    const settleList = _ocContractSettlList
      .concat(_ocDiscount)
      .concat(_ocPoints)
      .map((item: typeof initialValueOrder) => {
        return {
          ...item,
    ...
    try {
      ...
      if (
        value.contractPmode + "" === "1" ||
        value.creditType == "100" ||
        _payMoney === 0
      ) {
        ...
        const { dataObj } = await post(
          "web/oc/contract/syncContractBatchState.json",
          {
